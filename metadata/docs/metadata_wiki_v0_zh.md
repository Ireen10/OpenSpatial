...
> 本节是运行层面的补充说明，不改变本页 v0 schema 定义。

### 8.1 训练导出默认行为（2026-04）

- 训练导出阶段默认启用流式写出（`pipeline_streaming_enabled: true`），用于降低导出峰值内存。
- 如需快速回退到历史行为，可在配置中设：
  - `pipeline_streaming_enabled: false`
- `row_align` 尾部不足的处理默认仍是兼容历史的丢弃模式：
  - `training_remainder_mode: drop`
  - 可选 `sidecar`（保留尾部余数到 sidecar 文件）

### 8.2 断点续跑与流程切换注意事项

- checkpoint 以输入文件进度为核心，不区分“老流程阶段一”和“新流程阶段二导出”。
- 若从旧流程切到新流程，且沿用同一 output/checkpoint 并直接 `resume=true`，可能出现样本被误判为“已处理”而跳过。
- 建议在流程切换时采用以下任一策略：
  1. 使用新的 `output_root` 重新跑；
  2. 或清理目标 split 的 `.checkpoints` 后再跑；
  3. 或以 `to_metadata=false` 从既有 metadata shard 继续，并避免继承旧 checkpoint 语义。
