...
```json
{
  "dataset": { "name": "human_anno", "version": "v0", "split": "train", "dataset_path": "metadata/templates/configs_minimal/datasets/human_anno/dataset.yaml" },
  "sample": {
...
| `qa_items` | 否 | 生成后的 QA 列表；未生成时为空数组 | QA 生成阶段 |
...
| `dataset.dataset_path` | `string?` | 可选：dataset 配置路径（用于追溯生成配置） | `"metadata/templates/configs_minimal/datasets/refcoco/dataset.yaml"` |
...
| `sample.image.width` | `int?` | 可选：宽（缺失时由上游决定是否补齐） | `640` |
| `sample.image.height` | `int?` | 可选：高（缺失时由上游决定是否补齐） | `480` |
...
| 字段名 | 类型 | 说明 | 示例 |
|---|---|---|---|
| `relations[].relation_id` | `string?` | 可选关系 id；缺失时 schema 侧会自动补齐（如 `relation#0`） | `"relation#0"` |
...
| `relations[].source` | `string?` | 可选来源标记：`"huashan_annotated"` \| `"computed"` \| `"imported"` | `"huashan_annotated"` |
...
### 6.7 `qa_items`（QA 列表，可选）

`qa_items` 对应 schema 中的 `AnnotationQaItemV0[]`。未生成 QA 时保持空数组。

| 字段名 | 类型 | 说明 | 示例 |
|---|---|---|---|
| `qa_items[].qa_id` | `string` | QA 主键 | `"qa#0"` |
| `qa_items[].task` | `string?` | 任务名（例如 `spatial_relation_2d`） | `"spatial_relation_2d"` |
| `qa_items[].question` | `string` | 问题文本（schema 默认空串） | `"Where is the chair relative to the table?"` |
| `qa_items[].answer` | `string` | 答案文本（schema 默认空串） | `"left"` |
| `qa_items[].question_type` | `string?` | 题型 | `"open_ended"` |
| `qa_items[].question_tags` | `string[]` | 标签列表（默认空） | `["2D Spatial Relation"]` |
| `qa_items[].meta` | `object` | 扩展元信息（默认空对象） | `{"relation_id":"relation#0"}` |
| `qa_items[].relation_id` | `string?` | 对应关系 id | `"relation#0"` |
